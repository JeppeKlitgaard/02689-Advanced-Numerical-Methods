...
Consider a general second-order equation, 
$
  a hat(u)^((2))_n + b hat(u)^((1))_n + c hat(u)_n= hat(f)_n,
$
and its rewritten version,
$  
  hat(u)^((2))_n =
...
$
  hat(u)^((1))_n = 1/(2n-1) hat(u)^((2))_(n-1) - 1/(2n+3) hat(u)^((2))_(n+1),\ hat(u)_n = 1/(2n-1) hat(u)^((1))_(n-1) - 1/(2n+3) hat(u)^((1))_(n+1).
$
...
#figure(
  image("output/1_a_col_eps0001.png"),
...
The plots show that the convergence is quite similar for the two methods and that they perform quite equally. As $epsilon$ grows, the convergence is much faster, getting to machine-precision around $N=25$ for $epsilon=0.01$ compared to $N=175$ for $epsilon=0.001$. To explain this difference, we look at the exact solutions, as plotted above. Clearly the solution is much steeper close to $x=0$ for smaller values of $epsilon$. This means that the solver will need a much finer grid to achieve enough precision in the estimation of the derivative close to $x=0$, explaining the much slower convergence rate.
...
The Legendre Tau Method results in a sparse system matrix, which means that it is more memory efficient. This is of course a large advantage, when we consider problems, such as the problem presented here for $epsilon=0.001$, where a large number of grid points is needed to get precise solutions.
...
For the $r$ coordinate, we employ the spectral Legendre collocation method which has canonical domain $z_r ∈ [-1, 1]$, which we may transform into our physical domain using the transformation:
$
r(z_r) = (r_2 - r_1)/2 z_r + (r_1 + r_2)/2
$
...
$
  partial / (partial t) u + 6 u partial / (partial x) u + partial^3 / (partial x^3) u = 0, quad -infinity < x < infinity, t > 0,
$
...
$
  u_N (x,t) = sum_(i=0)^N hat(u)_i (t) phi.alt_i (x), quad phi.alt_i (x) = e^(i (2 pi n)/(2alpha) x )=e^(i (pi n)/(alpha) x )
$

IN JEPPE CODE THIS IS DONE DIFFERENTLY RIGHT?

We use the Fourier differentiation matrix from assignment 1 with explicit formulas for Lagrange polynomials, $D$.
...
$
  partial_x u_N (x_j, t)=sum_(i=0)^N hat(u)_i (t) h_i ^' (x_j), quad partial_(x x x) u_N (x_j, t)=sum_(i=0)^N hat(u)_i (t) h_i ^''' (x_j),
$
and insert this into the equation. Defining $vv(u)=[hat(u)_0 (t), hat(u)_1 (t),..., hat(u)_N (t)]$, we get a system of ordinary differential equations,
$
  partial / (partial t) vv(u) = -6"diag"(vv(u)_N) D vv(u)_N - D^3 vv(u)_N,
$
with initial condition
$
  vv(u)_0 = [f(x_0), f(x_1),..., f(x_N)].
$

The system is clearly non-linear, so we apply the technique of frozen coefficients from @L7_slides[slide 36], where we approximate the system matrix as
$
  L_N approx - 6 (max_(i in {0, ... ,N}) |u_N (x_i, 0)|) D - D^3,
$
which gives us a linear system, which can be solved by any ODE solver. For this project we use `solve_ivp` form SciPy's `integrate` module with it's standard numerical solver `RK45`, which is an explicit Runge-Kutta method of order 5(4), @solve_ivp.


To ensure that we get a stable solver, we have to control the time stepping. Specifically, we have to control the size of each time step, $Delta t$, such that @L6_slides[slide 40]
$
  Delta t <= s /(C N^p), quad lambda_"max" (L_N)<=C N^p.
$
We have found that for Runge-Kutta 4 $s=2.7853$. // TODO: source here, is it a webpage??
For the $p$ value, we have $|lambda_"max"|=max|i|=N$ for $D$, @L7_slides[slide 37]. In this case, we have $D^3$ in our system, so we must have $p=3$. To find a value for $C$ and confirm the value for $p$, we plot $max_N |lambda_N|$, see @fig:2_max_eigvals
...
The plot confirms that $p=3$ and furthermore, we see that choosing $C=3 dot 10^(-4)$ ensures  $lambda_"max" (L_N)<=C N^p$  for all large values of $N$ as we want. 
...
None of the solutions seem to converge, the error grows after $N=50$. This might stem from the fact that we consider the solution in a too small finite interval, which means that our periodic boundary conditions are not satisfied. We therefore try to solve the problem with $alpha=50$, see @fig:2_d_alpha50.
...
E&=integral_(-infinity)^infinity u^2(x) dif x approx sum_(i=1)^N ((1/2 (u_x (x_(i-1)))^2-(u(x_(i-1)))^3)+((1/2 (u_x (x_(i)))^2-(u(x_i))^3)))/2 Delta x_i=tilde(E).
$
In the code, we use the function `trapezoidal` from SciPy's `integrate` module.
...
$
u(x) &= sum_(k=-N/2)^(N/2-1) tilde(u)_k e^(i k x)\
v(x) &= sum_(k=-N/2)^(N/2-1) tilde(v)_k e^(i k x)\
$

As such, the element-wise multiplication becomes a convolutional sum in the modal space, which by construction will lead to the creation of higher-order modes, which are _aliased_ when restricted to the original lower-order discretisation. This aliasing effect pollutes the low frequency modes. 

In order to visualise this effect 


To de-alias, we will use Orzag's 3/2 de-aliasing rule, @L7_slides[slides 21-22]. For this, we introduce a longer vector of length $M$, which is zero-padded, that is 
$
hat(hat(u))_i=cases(
  hat(u)_i\, & quad i = 0\,...\,N,
  0\,& quad "otherwise"
...
$
To avoid aliasing in the modes, we are interested in, that is modes $i=0,...,N$, we need $M>= 3/2 N$.
...
To get a better understanding the scalability of our solver implementation, we benchmark a version where the differentiation is done by carrying out a Fourier transform such that the convolutional sum of the differentiation may be undertaken in $cal(O)(N)$ time as a element-wise multiplication in the dual domain, after which it is returned to the nodal domain with an inverse Fourier transformation.
...
This approach approach yields the scaling seen in @fig:2g_internal, which notable does _not_ follow the expected scaling. This is attributed to significant and variable overhead in the time-stepping algorithm, as well as the analysis being carried out at relatively low $N$ where linear or constant time operations may still dominate the performance.
...
We note that in both cases the matrix-based solver has a step in performance at $N=64$, which we theorise to arise from exceeding a cache-level or no longer fitting in a SIMD operation. This also highlights the importance of operations with below quadratic scaling at low $N$, which causes the matrix-based implementation to outperform the FFT-based implementation for $N≤64$.
...
== Solving an IVP as a BVP <sec:2.h>

// I DID IT AS I HAVE DONE IN CODE - SO DEFINE DOMAIN [-ALPHA, ALPHA]

We consider the linear advection equation which is a Initial Value Problem
$
  pdv(phi, t) + a pdv(phi, x) = 0, quad x in [0, 2pi] \
  phi(x, 0) = phi_0(x) quad x in [0, 2pi] \
  phi(0, t) = g_l (x) quad t >= 0 \  
$
We choose to consider the soliton function from section 2c as the solution to the linear advection equation, restricted to the domain $[0, 2pi]$ and we further derive from it the boundary conditions $phi_0, g_l$. The function is the following
$
  phi(x, t) = f(x - c t), quad f(x)=1/2 c sech^2(1/2sqrt(c)(x-x_0)), quad x in [0, 2pi]
$
We wish to solve the Initial Value Problem as the Boundary Value Problem with the spectral Legendre Collocation Method. We consider the formulation of the linear advection equation as the Boundary Value Problem
$
  pdv(phi, t) + a pdv(phi, x) = 0, quad (x, t) in [0, 2pi] times [0, T_max]\
  phi(x, 0) = phi_0(x) quad x in [0, 2pi] \
...
$
We consider the nodal expansion
$
  phi_N (z_x, z_t) = sum_(i=0)^N_x sum_(j=0)^N_t phi_(i j) h_i (z_x) h_j (z_t)
$
where the grid points are the Jacobi Gauss-Lobatto grid points. We use the spectral Legendre Collocation Method, and we therefore want the residual to be zero at each collocation node,
$
...
$
  pdv(phi_N, z_x)(z_x, z_t) = D_(z_x) bold(phi), quad pdv(phi_N, z_t)(z_x, z_t) = D_(z_t) bold(phi), quad D_(z_x) in R^(N_x times N_x), quad D_(z_t) in R^(N_t times N_t)
$
We construct the global operator by using the kronecker products with the Identity matrices
$
  L_N = ((2 / (T_max) D_(z_t)) times.o I_(z_x)) + (( a / (pi) D_(z_x)) times.o I_(z_t))
$
...
We note the relationship between the temporal discretization $N_t$ and the spatial discretization $N_x$. Based on the plots it could be estimated that $N_x > gamma N_t$ where $gamma approx 4$. It was verified that $gamma$ does not depend on the problem parameters. Consider the convergence curve for fixed $N_t$ and as a function of $N_x$. When $N_x < gamma N_t$ the spatial error dominates the overall error, since the temporal dimension have been discretized enough. Once $N_x > gamma N_t$, further spatial refinement do not result in smaller norm of the error, which indicates that the temporal error dominates. This statement is further supported by the fact that with higher fixed $N_t$ the plateau in the convergence plot is seen on lower values of the norm of the error. 
...
#figure(
  ...
  caption: [
    Snapshots of the solutions for the linear advection equation.
  ]
